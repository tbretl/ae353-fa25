...
class Simulator:
    ...
    def place_rings(self):
        ...
        z = self.rng.choice(z_options, 6, replace=False)
    ...
    def add_dog(self, pos, ori=[0, 0, 0], urdf='dog.urdf'):
        if len(ori) == 3:
            # ori is rpy
            q = self.bullet_client.getQuaternionFromEuler(ori)
        elif len(ori) == 4:
    ...
    def place_dog_pack(self, p1, p2, d_n=0.5, d_z=1.0):
        z = np.array([0., 0., 1.])
        v = (p2 - p1) / np.linalg.norm(p2 - p1)
        n = np.cross(z, v)
        for s in [0.4, 0.5, 0.6]:
            p = (1 - s) * p1 + s * p2
            p += self.rng.uniform(-d_n, d_n) * n
            p += self.rng.uniform(-d_z, d_z) * z
            ori = self.rng.uniform([-0.5, -0.5, -1.5], [0.5, 0.5, 1.5])
            self.add_dog(p, ori=ori)

    def place_dogs(self):
        # Make sure rings exist
        assert(len(self.rings) > 0)

        # Remove all existing dogs
        for dog in self.dogs:
            self.bullet_client.removeBody(dog['id'])
        self.meshcat_clear_dogs()
        self.dogs = []
        ...
        location_options = [
            ...
            {
                'p1': self.rings[6]['p'] + np.array([0.5, 0., 0.]),
                'p2': self.rings[7]['p'] + np.array([0.5, 0., 0.]),
            },
            {
                'p1': self.rings[7]['p'],
                'p2': self.rings[8]['p'],
        ...
        for location in self.rng.choice(location_options, 3, replace=False):
            self.place_dog_pack(location['p1'], location['p2'])
        
        # Set contact parameters for all rings
        for dog in self.dogs:
            self.bullet_client.changeDynamics(dog['id'], -1,
                    lateralFriction=1.0,
                    spinningFriction=0.0,
                    rollingFriction=0.0,
                    restitution=0.5,
        ...
        # Add all dogs to meshcat
        self.meshcat_add_dogs()
    ...
    def reset(
        ...
        if initial_conditions is None:
            ...
            # - this is what we would have done if there were more than two drones
            # p = self._get_points(len(self.drones), 0.25, 2.5)
            # - this is what we will do when we know there are at most two drones
            p = np.array([
                [0., 0.5], # left
                [0., -0.5], # right
            ])
            p = self.rng.choice(p, len(self.drones), replace=False)
            if p is None:
    ...
    def run(
        ...
        if videos is not None:
            ...
            for video in videos:
                ...
                if 'start_frame' in video.keys():
                    # Add title frame
                    rgba = video['start_frame']['callback'](self.drones)
                    for i in range(int(video['start_frame']['length'] * fps)):
                        video['writer'].append_data(rgba)
                    # Add first frame of simulation
                    rgba = self.snapshot(video['view_name'])
                    for i in range(int(video['start_frame']['pause'] * fps)):
                        video['writer'].append_data(rgba)
                else:
                    # Add first frame of simulation
                    rgba = self.snapshot(video['view_name'])
        ...
        if videos is not None:
            for video in videos:
                # Create end of video
                if 'finish_frame' in video.keys():
                    # Add last frame of simulation
                    for i in range(int(video['finish_frame']['pause'] * fps)):
                        video['writer'].append_data(rgba)
                    # Add title frame
                    rgba = video['finish_frame']['callback'](self.drones)
                    for i in range(int(video['finish_frame']['length'] * fps)):
                        video['writer'].append_data(rgba)
    ...
    def step(self, print_debug=False):
        ...
        # get position of all drones and dogs (and index of inactive drones)
        all_pos = []
        index_inactive = []
    ...
    def meshcat_add_dogs(self):
        for view in self.views:
            self._meshcat_add_dogs(view['vis'])

    def _meshcat_add_dogs(self, vis):
        for dog in self.dogs:
            s = self.bullet_client.getVisualShapeData(dog['id'])[0]
            if s[1] != -1:
                raise Exception('bad dog link id')
            stl_filename = s[4].decode('UTF-8')
            scale = s[3]
            color = self._convert_color(s[7])
            dog['name'] = f'{dog["id"]}'
            dog['scale'] = scale
            # - Object
            vis['scene']['dogs'][dog['name']].set_object(
                meshcat.geometry.StlMeshGeometry.from_file(stl_filename),
                meshcat.geometry.MeshPhongMaterial(
                    color=color['color'],
                    transparent=color['transparent'],
            ...
            # - Transform
            pos, ori = self.bullet_client.getBasePositionAndOrientation(dog['id'])
            S = np.diag(np.concatenate((dog['scale'], [1.0])))
            T = meshcat.transformations.quaternion_matrix(self._wxyz_from_xyzw(ori))
            T[:3, 3] = np.array(pos)[:3]
            vis['scene']['dogs'][dog['name']].set_transform(T @ S)

    def meshcat_clear_dogs(self):
        for view in self.views:
            self._meshcat_clear_dogs(view['vis'])

    def _meshcat_clear_dogs(self, vis):
        vis['scene']['dogs'].delete()
