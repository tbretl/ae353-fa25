...
class Simulator:
    def __init__(
        ...
        for object_id in [self.robot_id, self.platform_id, self.ring_id]:
    ...
    def has_landed(self):
        ...
        # Z component of position must be near the top of the platform
        if p_z < 14.75:
            return False
        if p_z > 15.00:
            return False
        
        # X and Y components of position must be inside the platform        
        ...
        # Pitch and roll angles must be near zero
        if np.abs(theta) > (np.pi / 12):
            return False
        if np.abs(phi) > (np.pi / 12):
            return False
